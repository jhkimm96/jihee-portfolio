...
export default async function TroubleshootingPage({ searchParams }: { searchParams: Promise<{ view?: string; project?: string }> }) {
  const params = await searchParams
  const view = params.view === 'latest' ? 'latest' : 'category'
  const project = params.project
  const posts = getPublishedTroubleshooting().filter((post) => !project || post.project === project)
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      {posts.length === 0 ? (
        ...
        <>
          <ViewToggle basePath="/troubleshooting" view={view} project={project} />
