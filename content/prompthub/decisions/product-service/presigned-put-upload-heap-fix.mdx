...
> **후속 결정:** presigned PUT 방식은 유지하되, 상품 저장 계약은 만료 URL 대신 `objectKey`를 사용하도록 변경했다. 실패 시 파일 정리 방식은 [업로드 미리보기 URL과 저장 키 분리](/decisions/prompthub/product-service/object-key-upload-and-transaction-compensation)에서 이어서 설명한다.
