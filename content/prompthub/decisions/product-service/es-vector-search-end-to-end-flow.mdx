...
flowchart TB
    ...
    IDS --> PGQ[(Postgres에서<br/>상세 정보 조회)]
    PGQ --> RES[응답]
...
합친다. **Postgres는 이 흐름에 등장하지 않는다** — 마지막에 상세 정보를 채울 때만 쓴다.
...
정직하게 남긴다. 지금 구조에 있는 빈틈이다.

- **인덱스 교체 절차가 없다.** 별칭(`products` → `products-v1`)이라는 간접층은 만들어 뒀지만,
  새 인덱스를 만들어 무중단으로 갈아끼우는 코드는 없다. 매핑을 바꾸려면 지금은 재색인이
  필요하다
- **고아 문서 탐지에 1만 건 상한이 있다.** 카드 상자에만 남고 서가에는 없는 문서를 찾을 때
  한 번에 1만 건까지만 조회한다. 카탈로그가 그보다 커지면 일부가 탐지에서 빠진다
- **리뷰 수는 항상 0이다.** 카드에 적히긴 하는데 집계 쿼리가 없어 `0`이 하드코딩돼 있다
- **가격 내림차순 정렬이 검색 경로에 없다.** 데이터베이스 경로에는 있지만 검색 엔진 경로에는
  없어서, 요청이 와도 인기순으로 바뀐다
