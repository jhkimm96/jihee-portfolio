...
> **후속 변경:** 반려된 major 수정본은 새 버전을 만들지 않고 같은 row·version에서 보정한 뒤 별도로 재요청한다. 판매자 대표 선택도 `PENDING_REVIEW → REJECTED → ON_SALE` 순서로 변경됐다. 자세한 원인과 검증은 [판매 중 버전에 가려진 반려 수정본을 같은 버전으로 복구](/troubleshooting/prompthub/product/rejected-major-same-version-edit)에서 설명한다.
...
stateDiagram-v2
    ...
    ON_SALE --> PENDING_REVIEW: 되돌리기
    ON_SALE --> SUPERSEDED: PATCH (즉시)
    ON_SALE --> SUPERSEDED: MAJOR 승인 시
    SUPERSEDED --> ON_SALE: 되돌리기 복원
    ON_SALE --> STOPPED: 판매 중단
```

- **PATCH 수정**: 새 row가 바로 ON_SALE, 기존은 **즉시** SUPERSEDED (검수 없음).
- **MAJOR 수정**: 새 row가 PENDING_REVIEW로 시작, 기존 ON_SALE은 유지 → **승인될 때** 밀림.
