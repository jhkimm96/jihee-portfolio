...
1. **"테스트 452개 전부 통과"가 실제로는 틀렸다.** Gradle `compileTestJava`가 테스트 소스
   ...
   드러났다. → 해결: [Gradle 캐시가 실패한 테스트를 통과로 보여줬다](/troubleshooting/prompthub/product/gradle-stale-test-cache-false-pass)
2. **10,001건 테스트가 단독 실행에서만 통과했다.** 공유 Testcontainers ES에 문서를 심어두고
   정리를 안 해서 무관해 보이는 다른 통합 테스트(정렬 검증)가 깨졌다. 클래스 단위 실행과
   전체 스위트 실행을 비교해 원인을 좁혔다. → 해결: [공유 테스트 컨테이너에 문서를 심어두고 안 지워서 다른 테스트가 깨졌다](/troubleshooting/prompthub/search/shared-es-testcontainer-pollution)
